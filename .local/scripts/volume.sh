...
send_notification() {
	...
	dunstify -a "changevolume" -u low -r "9997" -h int:value:"$volume" -i "$icon" "Volume: ${volume}%" -t 2000
...
mute)
	...
	if eval "$(pamixer --get-mute)"; then
		dunstify -i notification-audio-volume-muted -a "changevolume" -t 2000 -r 9997 -u low "Muted"
	else
